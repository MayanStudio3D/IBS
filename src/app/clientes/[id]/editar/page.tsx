...
export default function EditarClientePage() {
  ...
  useEffect(() => {
    async function fetchCliente() {
      if (!id) return;
      try {
        const { data, error } = await supabase.from('ibs_clientes').select('*').eq('id', id).single();
        ...
        if (data) {
          ...
          try {
            parsedAddress = JSON.parse(data.endereco || '{}');
             ...
             // fallback caso já exista endereco como string simples
             parsedAddress = { logradouro: data.endereco };
          }
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700 p-6 lg:p-12 pb-32">
      <header className="flex items-center gap-6">
        ...
        <div>
          <h2 className="text-3xl lg:text-4xl font-black text-[#D4AF37] tracking-tighter uppercase italic">Editar Cliente</h2>
          <p className="text-gray-500 font-bold uppercase tracking-[0.2em] text-[10px] md:text-xs">Atualize os dados e endereço do cliente</p>
        </div>
      </header>
