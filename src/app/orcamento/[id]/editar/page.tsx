...
export default function EditarOrcamentoPage() {
  ...
  useEffect(() => {
    async function loadData() {
      ...
      if (id) {
         const { data: pedido } = await supabase.from('ibs_pedidos').select('*, ibs_pedido_itens(*)').eq('id', id).single();
         if (pedido) {
            setSelectedCliente(pedido.cliente_id);
            setCondicaoPgto(pedido.condicao_pagamento || 'A VISTA');
            setVendedorNome(pedido.vendedor_nome || '');
            if (pedido.condicao_pagamento === 'PARCELADO' && pedido.parcelas?.length > 0) {
              setQtdParcelas(pedido.parcelas.length);
            }
            if(pedido.validade_data) setValidade(pedido.validade_data);
            if(pedido.ibs_pedido_itens && pedido.ibs_pedido_itens.length > 0) {
               setItems(pedido.ibs_pedido_itens.map((i: any) => ({
                 id: i.id || Math.random().toString(),
                 descricao: i.descricao,
                 vol: i.volume || 1,
                 comp: i.comprimento || 1,
                 alt: i.altura || 1,
                 preco_unitario: i.preco_unitario || 0,
                 foto_url: matData?.find(m => m.nome === i.descricao)?.foto_url || ''
               })));
            }
         }
      }
    ...
    // Set default validity apenas se for novo
    if(!id){
      const date = new Date();
  ...
  const handleSave = async () => {
    ...
    if (items.some(i => !i.descricao)) {
      setError('Todos os itens precisam de uma descrição.');
      return;
    }
    ...
    try {
      ...
      // Excluir Itens Antigos e Recriar para fácil update complexo com arrays dinâmicos
      const { error: delErr } = await supabase.from('ibs_pedido_itens').delete().eq('pedido_id', id);
        ...
        <div className="bg-[#1A1A1A] w-full max-w-4xl max-h-[90vh] overflow-hidden rounded-2xl border border-white/10 shadow-2xl flex flex-col">
          <header className="p-6 border-b border-white/5 flex items-center justify-between bg-[#121212]">
            <div className="flex items-center gap-3">
              ...
              <h3 className="text-xl font-black text-white uppercase tracking-tight">Conferência de Documento</h3>
            </div>
          ...
          <div className="flex-1 overflow-y-auto p-8 space-y-8 bg-white text-black font-sans text-left">
            ...
            <div className="flex justify-between items-start border-b-2 border-gray-100 pb-6 text-black">
              ...
              <div className="text-right flex items-center gap-4">
                <div>
                   <p className="text-sm font-bold">Orçamento #{(id || '').substring(0, 8).toUpperCase()}</p>
            ...
            <div className="grid grid-cols-2 gap-8 text-sm text-black">
              ...
              <div className="text-right">
                ...
                <div className="mt-4">
                  <p className="text-[10px] uppercase font-black text-gray-400 tracking-widest">Vendedor</p>
                  <p className="font-bold uppercase">{vendedor_nome || 'Sistema'}</p>
                </div>
              </div>
            </div>
            ...
            <div className="mt-12 pt-8 border-t-2 border-dashed border-gray-200">
               <h4 className="text-center font-black text-xs uppercase tracking-[0.2em] text-gray-400 mb-6 font-sans">Protocolo de Aprovação</h4>
               <div className="w-48 h-px bg-gray-300 mx-auto mb-2" />
               <p className="text-center text-[10px] text-gray-400 uppercase font-bold">Assinatura do Cliente</p>
            </div>
          </div>

          <footer className="p-6 bg-[#121212] flex gap-4">
             <button onClick={() => setShowPreview(false)} className="flex-1 py-4 border border-white/10 rounded-xl text-gray-400 font-black uppercase text-[10px] tracking-widest">Voltar para Edição</button>
             <button onClick={() => { setShowPreview(false); handleSave(); }} className="flex-[2] py-4 bg-[#D4AF37] text-black rounded-xl font-black uppercase text-[10px] tracking-widest flex items-center justify-center gap-3 transition-all hover:scale-[1.01] active:scale-95">
                <Save size={18} /> Confirmar e Atualizar
             </button>
          </footer>
        </div>
      </div>
    ...
    <div className="max-w-[1600px] mx-auto p-4 md:p-6 space-y-6 animate-in fade-in duration-700 pb-24">
      ...
      <header className="bg-[#1A1A1A] p-6 rounded-3xl border border-white/5 shadow-2xl flex flex-col md:flex-row items-center gap-6 justify-between">
        <div className="flex items-center gap-5">
          ...
          <div>
            <h2 className="text-2xl font-black text-white tracking-tighter uppercase italic">Editar Orçamento</h2>
            <div className="flex items-center gap-2 mt-1">
              <span className="w-2 h-2 rounded-full bg-blue-500 animate-pulse" />
              <p className="text-gray-500 font-bold uppercase tracking-widest text-[9px]">Protocolo #{(id || '').substring(0, 8).toUpperCase()}</p>
            </div>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        <div className="lg:col-span-3 space-y-6">
          ...
          <section className="bg-[#1A1A1A] p-6 rounded-3xl border border-white/5 shadow-xl grid grid-cols-1 md:grid-cols-4 gap-6">
            ...
            <div className="space-y-2">
              <label className="text-[10px] uppercase font-black text-gray-500 tracking-widest ml-2 flex items-center gap-2">
                <Clock size={14} className="text-[#D4AF37]" /> Protocolo ID
              </label>
